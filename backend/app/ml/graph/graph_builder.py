...
class GraphBuilder:
    ...
    def from_campaign_data(
        ...
        for acc in accounts:
            ...
            # Edge: Account -> Platform (ON_PLATFORM)
            plat_id = str(acc.get("platform_id", ""))
            if plat_id and plat_id in G:
                G.add_edge(a_id, plat_id, type="REGISTERED_ON", label="ON_PLATFORM")

        # Add Posts and link to accounts & platforms
        for post in posts:
            post_id = str(post["id"])
            G.add_node(
                post_id,
            ...
            # Edge: Post -> Campaign (BELONGS_TO)
            G.add_edge(post_id, camp_id, type="BELONGS_TO", label="IN_CAMPAIGN")
        ...
        # Add dynamic propagation events
        for ev in events:
