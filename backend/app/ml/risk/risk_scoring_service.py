...
class RiskScoringService:
    ...
    def calculate_risk_score(
        ...
        # 1. AI Generation Factor
        ai_factor = min(1.0, max(0.0, ai_probability))
        score_components["ai_generation_factor"] = round(ai_factor * 0.25, 3)
        if ai_factor >= 0.80:
            reasons.append(f"High synthetic content probability ({int(ai_factor * 100)}%) detected by linguistic classifier.")
        elif ai_factor >= 0.50:
            reasons.append(f"Moderate synthetic content characteristics ({int(ai_factor * 100)}%) observed.")

        # 2. Velocity & Branching Factor
        ...
        if propagation_velocity > 10.0:
            reasons.append(f"Rapid propagation velocity ({propagation_velocity:.1f} events/hour) surpassing baseline organic dissemination.")
        if branching_factor >= 3.0:
            reasons.append(f"High branching factor ({branching_factor:.1f}) indicating automated multi-branch cascade amplification.")

        # 3. Cross-Platform Dispersion
        if platform_count >= 4:
            plat_factor = 1.0
            reasons.append(f"Broad synchronized cross-platform deployment across {platform_count} distinct platforms.")
        elif platform_count == 3:
            plat_factor = 0.75
            reasons.append(f"Multi-platform propagation active across {platform_count} platforms.")
        elif platform_count == 2:
            plat_factor = 0.40
            reasons.append("Cross-platform reposting detected across 2 platforms.")
        else:
            plat_factor = 0.10
        ...
        if derived_variant_count >= 5:
            deriv_factor = 1.0
            reasons.append(f"Extensive narrative mutation with {derived_variant_count} derived paraphrase variants.")
        elif derived_variant_count >= 2:
            deriv_factor = 0.60
            reasons.append(f"Multiple paraphrased content variants ({derived_variant_count}) circulating simultaneously.")
        elif max_similarity_to_known_campaign >= 0.80:
            deriv_factor = 0.70
            reasons.append(f"Strong semantic similarity ({int(max_similarity_to_known_campaign * 100)}%) to existing tracked campaign.")
        else:
            deriv_factor = 0.05
        ...
        if gnn_factor >= 0.70:
            reasons.append(f"GNN graph topology analysis identified coordinated inauthentic behavior signature (score {gnn_factor:.2f}).")
        if bot_account_ratio >= 0.50:
            reasons.append(f"High concentration of automated/synthetic accounts ({int(bot_account_ratio * 100)}%) participating in cascade.")
        ...
        if not reasons:
            reasons.append("Benign propagation metrics with low synthetic likelihood and organic distribution patterns.")
